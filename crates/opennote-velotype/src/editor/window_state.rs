...
impl Editor {
    ...
    /// falls outside the run. `strides[i]` is row `i`'s footprint (height plus
    /// trailing gap); being scroll-invariant, their running sum places each row
    /// against a band from the current scroll offset.
